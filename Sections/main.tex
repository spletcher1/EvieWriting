...
We focused our investigation on ER2 and ER4 neurons, with particular emphasis on the ER4d subpopulation, as these are required for the perception of dead conspecifics to modulate lifespan \citep{gendron2023} and for food perception to enhance starvation resistance \citep{luo2021}. 
...
Activation of ER4d neurons had a modest but significant effect on lifespan (\figref{fig:lifespansupp}A). 
By contrast, inhibition of ER4d neurons by exposing flies expressing the anion channelrhodopsin GtACR1 \citep{mohammad2017} (ER4d>GtACR1) to green light during subjective day produced a more pronounced lifespan extension in both females and males (\figref{fig:lifespanfig}C).
...
The observation that activation of ER2 neurons and, to a lesser extent, ER4d neurons increased lifespan was somewhat unexpected, given that measures of ER2 and ER4 neuronal activity were both observed to be higher following exposure to dead conspecifics \citep{gendron2023}, which accelerates aging \citep{chakraborty2019}. 
...
Using a driver that targets additional EB neuron populations, including at least R3 neurons, we observed that broader EB inhibition had a modest effect on lifespan (\figref{fig:lifespanfig}E, left) and activation increased lifespan to an equal or lesser extent than ER2 or ER2/ER4 activation alone (\figref{fig:lifespanfig}E, right). 

Together, these data demonstrate that manipulation of ER2 and ER4 neural activity increases lifespan, supporting the notion that the ellipsoid body is an instructive modulator of aging. 
...
After one week of ER2 neuron activation, we identified 123 genes (70 downregulated and 53 upregulated) whose expression values exhibited experiment-wise interaction p-values (corrected for false discovery rate) less than our cutoff of p < 0.01. 
...
Hierarchical clustering revealed that samples from flies with activated ER2 neurons (i.e. ER2>Chr flies exposed to red light) formed a distinct group from flies without neuronal activation, including CsChrimson-expressing flies maintained in constant darkness as well as control genotype samples maintained in either darkness or red light. 

Despite a stringent threshold, we observed that one week of ER2 neuron activation drove broad but pathway-enriched changes in the head. 
Gene Ontology (GO) analysis of upregulated genes revealed enrichment for ribosome biogenesis and rRNA processing, insulin signaling, and response to starvation (\figref{fig:seqfig}B). 
...
Notably, many of the differentially-expressed genes have been implicated in aging biology, with several known to modulate aging directly. 
...
Following one week of ER4d silencing, far fewer changes in gene expression could be attributed to ER4d inhibition (\figref{fig:seqfig}D), although such changes were likely masked by remarkably potent independent effects of green light exposure. 
...
However, only 11 genes (9 downregulated and 2 upregulated) exhibited a significant genotype $\times$ light interaction, indicating a differential transcriptional response in flies with ER4d neurons silenced. 
...
A GO analysis of the upregulated genes was dominated by metabolic pathways related to amino acid metabolism as well as pentose-phosphate- and NADPH-generating processes, with additional enrichment of biosynthetic and glycogen-metabolism-related pathways (\figref{fig:seqfig2}B). 
...
We therefore asked how ER2 activation, ER4d inhibition, and ER2/ER4 co-activation affected feeding, energy storage and mobilization, and organismal performance, phenotypes expected to track changes in nutrient state, motivational drive, and overall physiological condition.
...
There were also no consistent effects of ER2/ER4 neuron activation on feeding, as the interaction term was not statistically significant for males or females that were light-naïve (\figref{fig:conexfig}B, left panels for each sex) or given one week (\figref{fig:conexfig}B, center) or one month 
...
\subsubsection*{Lipid stores and starvation resistance do not correlate with transcriptional signatures of ER2 and ER2/ER4 activation}
...
After one week of ER2 activation to match RNA-seq treatment, TAG levels were unchanged in females but significantly reduced in males (\figref{fig:conexfig}C), whereas ER2/ER4 activation significantly reduced TAG in both sexes (\figref{fig:conexfig}D), contrary to the observed decrease in starvation-response genes. 
We then examined if TAG differences predicted survival under nutrient deprivation. 
Flies of both sexes were pre-treated with light for one week to match TAG assay conditions before being flipped onto 2\% agar, a hydrating but non-nutritive medium. 
Females of both genotypes were more sensitive to starvation than controls (\figref{fig:conexfig}E-F, left panels). 
In males, starvation outcomes diverged despite overlapping effects on TAG: ER2>Chr males were only modestly sensitized to starvation compared to their dark controls, an effect driven by a long tail in control survival. By contrast, ER2/ER4>Chr males were significantly sensitized (\figref{fig:conexfig}E-F, right panels). 
...
Ring neurons have established roles in locomotor control \citep{buhl2021,kottler2019}, so we next asked whether an evoked motor assay might reveal differences among neural manipulations that measures of baseline feeding, TAG storage, and starvation resistance did not.
To assess this, we tested negative geotaxis, a stereotyped climbing response widely used to measure age-sensitive locomotor performance and neuromuscular function in \textit{Drosophila} \citep{rhodenizer2008, jones2009}. 
Flies were assayed using our custom “D-Drop” platform, which drops a chamber containing multiple single-fly lanes and uses video tracking to quantify several characteristics of flies' subsequent climbing behavior. 
...
ER2>Chr flies of neither sex exhibited a significant genotype $\times$ light interaction for either metric after one week or one month of activation (\figref{fig:ddropfig}A). 
ER2/ER4 activation likewise produced no detectable effect after one week of pre-treatment in either sex (\figref{fig:ddropfig}B, left panel for each sex). 
After one month of pre-treatment, however, female ER2/ER4>Chr flies outperformed controls in both height at 9 sec and total upward distance (\figref{fig:ddropfig}B). Males showed a modest decrease in climbing ability (Total Up p = 0.045), but this effect did not survive multiple testing correction.  
...
In females, we saw a modest trend toward reduced climbing ability after one week of ER4d inhibition (Total Up p = 0.035); however, after multiple testing correction, there were no detectable effects at either time point (\figref{fig:ddropfig}C). 
...
Our results suggested that EB ring neurons act instructively, imposing distinct internal states whose immediate consequences are separable but whose downstream effects converge on lifespan regulation. 
...
We therefore next asked whether these manipulations differ in valence, reasoning that their attractive or aversive properties might clarify the functional logic by which EB circuits influence lifespan.
...
This design allowed us to ask whether activation or inhibition of specific EB neuron populations altered the perceived reward value of the sucrose-associated food while accounting for non-associative consequences of light exposure (increased hunger, for example). 
...
Although trigger flies expressing GtACR1 in ER4d neurons displayed aversion to the sucrose well, a similar response was observed in trigger flies of the control genotype, precluding a clear assignment of valence to ER4d inhibition itself (\figref{fig:flicsupp}C). 
...
ER2/ER4-activated trigger flies continued to interact more with the sucrose well than their yoked controls after one week and one month of pre-treatment; however, at these later time points, there was no longer a significant (at p < 0.05) genotype $\times$ light effect (\figref{fig:flicfig}D, center and right, respectively), perhaps due to an increase in among-fly variance.
Nevertheless, ER2/ER4 flies showed a trend toward greater attraction to the well than controls at these later time points (p = 0.06–0.1).
...
Light-naïve flies of both sexes were tested, and each manipulation was evaluated at two stimulation frequencies (48 Hz to mimic FLIC conditions, and 200 Hz) to further gauge generalizability. 

The positive valence of ER2/ER4 co-activation generalized cleanly. 
Both sexes significantly preferred the illuminated side during treatment, an effect observed at both 48 Hz (\figref{fig:arenafig}B) and 200 Hz (\figref{fig:arenasupp}A), indicating that the appetitive state reflects a stable property of the ER2/ER4-activated circuit rather than an artifact of a particular assay or stimulation regime. 
...
In males, ER2 activation produced no preference at 48 Hz (\figref{fig:arenafig}C, right) or 200 Hz (\figref{fig:arenasupp}B, right). 
...
ER4d>GtACR1 and control flies showed indistinguishable side biases (females preferring illumination, males avoiding it; \figref{fig:arenasupp}C), so although the open-loop design removed the light-startle confound, it did not permit an unambiguous valence assignment. 
ER4d activation, which had been aversive for naïve flies in the FLIC, produced variable behavior in the arena. 
ER4d activation produced an aversive shift in both sexes at 48 Hz (\figref{fig:arenafig}D, left), although the effect was stronger in males. 
Females exhibited reduced light preference relative to controls, whereas males preferred the unlit side.
Neither sex exhibited a significant place preference at 200 Hz (\figref{fig:arenasupp}D, left).
...
ER2/ER4 co-activation produced a robust positive valence that generalized across sex and assay paradigms, whereas ER2 and ER4d manipulations did not yield consistently generalizable valence signatures. 
...
Finally, to examine how EB neuron manipulation in the brain influences gene expression and physiology in the periphery, we performed bulk RNA-sequencing and metabolomic profiling on headless bodies of female flies. 
We focused on ER2-activated flies to determine whether the strong starvation-response program observed in heads would manifest in the periphery. 
...
Again, using DESeq2 to identify genes with a significant genotype (CsChrimson, control) $\times$ light (yes, no) interaction, we identified 229 upregulated and 181 downregulated genes in peripheral tissues (experiment-wise p-value < 0.01) (\figref{fig:bodyseqsupp}A). 
